...
import org.dbpedia.lookup.indexer.LuceneIndexWriter;
...
public class LookupIndexer {
	...
	public void run(String[] values) throws IOException {
	 ...
	 */
	private void indexSparqlEndpoint() {

		LuceneIndexWriter lookupIndexer = new LuceneIndexWriter(config, logger);
	...
	private void buildMem() {
		...
		// Start the indexing
		LuceneIndexWriter lookupIndexer = new LuceneIndexWriter(config, logger);
		indexData(dataset, lookupIndexer);
	}
	...
	private void buildDisk() {
		...
		// Do the indexing
		LuceneIndexWriter lookupIndexer = new LuceneIndexWriter(config, logger);
		indexData(DatasetFactory.wrap(datasetGraph), lookupIndexer);
	}
	 ...
	 * @param indexConfig
	 */
	private void indexData(Dataset dataset, LuceneIndexWriter lookupIndexer) {
