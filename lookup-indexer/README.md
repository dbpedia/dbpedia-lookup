...
We want to index the DBpedia knowledge graph

 We write a SPARQL query and run it against a knowledge graph to receive a binding with two entries: The key *<https://dbpedia.org/resource/Berlin>* and the value *"Berlin"*. We use this query in our indexing process. Later, a user quering the index structure for **"Berl"** would then be able to retreive the indexed key *<https://dbpedia.org/resource/Berlin>* as his query string partially matches the indexed value *"Berlin"*.
